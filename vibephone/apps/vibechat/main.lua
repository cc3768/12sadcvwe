...
-- (also works if installed elsewhere, because we pass the entry path as argv[1])
...
local function clampScroll(maxLines, viewLines)
  local maxScroll = math.max(0, maxLines - viewLines)
...
local function uiLoop()
  ...
  while true do
    ...
    elseif e == "mouse_scroll" then
      ...
      if a == 1 then state.scroll = (state.scroll or 0) + 3
      else state.scroll = math.max(0, (state.scroll or 0) - 3) end

      clampScroll(total, viewLines)
      redraw()
    ...
    elseif e == "key" then
      ...
      elseif a == keys.pageUp or a == keys.pageDown then
        local w,h = term.getSize()
        local viewLines = (h-2) - 5 + 1
        ...
        if a == keys.pageUp then state.scroll = (state.scroll or 0) + 3
        else state.scroll = math.max(0, (state.scroll or 0) - 3) end
