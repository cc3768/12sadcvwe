-- VibePod (iPod app) for VibePhone
-- Goal: behave like the ORIGINAL script, but run inside the VibePhone app sandbox without crashing,
-- and without the "Must specify http or https" / network URL issues.

local function run()
  local config = require("config")

  -- Keep the original semantics: api_base_url already includes trailing "/"
  local api_base_url = tostring(config.API_BASE_URL or "https://ipod-2to6magyna-uc.a.run.app/")
  local version = tostring(config.VERSION or "2.1")

  -- Normalize: trim spaces
  api_base_url = (api_base_url:gsub("%s+$",""))

  -- Ensure scheme (fixes: "Must specify http or https")
  if not api_base_url:match("^https?://") then
    api_base_url = "https://" .. api_base_url
  end

  -- Keep original style: base ends with "/" so api_base_url .. "?v=..."
  if api_base_url:sub(-1) ~= "/" then api_base_url = api_base_url .. "/" end
  ...
  local function stopAll()
    for _, sp in ipairs(speakers) do pcall(function() sp.stop() end) end
  ...
  local function redrawScreen()
    ...
    for i = 1, #tabs do
      ...
      local x = (math.floor((width / #tabs) * (i - 0.5))) - math.ceil(#tabs[i] / 2) + 1
      if x < 1 then x = 1 end
      if x > width - #tabs[i] + 1 then x = width - #tabs[i] + 1 end
    ...
    local function drawNowPlaying()
      if now_playing ~= nil then
        ...
        term.write(tostring(now_playing.name or "Unknown"))

        term.setTextColor(colors.lightGray)
        term.setCursorPos(2, 4)
        term.write(tostring(now_playing.artist or ""))
      else
        term.setBackgroundColor(colors.black)
        term.setTextColor(colors.lightGray)
        term.setCursorPos(2, 3)
      ...
      local fillW = math.floor(24 * (volume / 3) + 0.5) - 1
      if fillW >= 0 then
        paintutils.drawBox(2, 8, 2 + fillW, 8, colors.white)
      end

      if volume < 0.6 then
        term.setCursorPos(2 + fillW + 2, 8)
        term.setBackgroundColor(colors.gray)
        term.setTextColor(colors.white)
      else
        term.setCursorPos(2 + fillW - 3 - (volume == 3 and 1 or 0), 8)
        term.setBackgroundColor(colors.white)
        term.setTextColor(colors.black)
      end
      ...
      if #queue > 0 then
        ...
        for i = 1, #queue do
          ...
          term.write(tostring(queue[i].name or "Unknown"))
          term.setTextColor(colors.lightGray)
          term.setCursorPos(2, 11 + (i - 1) * 2)
          term.write(tostring(queue[i].artist or ""))
        end
      end
    end
    ...
    local function drawSearch()
      ...
      if search_results ~= nil then
        ...
        for i = 1, #search_results do
          ...
          term.write(tostring(search_results[i].name or "Unknown"))
          term.setTextColor(colors.lightGray)
          term.setCursorPos(2, 8 + (i - 1) * 2)
          term.write(tostring(search_results[i].artist or ""))
        end
      else
      ...
      if in_search_result == true then
        ...
        term.setTextColor(colors.white)
        term.write(tostring(search_results[clicked_result].name or "Unknown"))
        term.setCursorPos(2, 3)
        term.setTextColor(colors.lightGray)
        term.write(tostring(search_results[clicked_result].artist or ""))
          ...
          function()
            ...
            if button == 1 then
              ...
              if in_search_result == false then
                if y == 1 then
                  if x < width / 2 then tab = 1 else tab = 2 end
                  redrawScreen()
                end
              end
      ...
      parallel.waitForAny(
        function()
          ...
          if url == last_search_url then
            -- CC:Tweaked uses "unserializeJSON"
            search_results = textutils.unserializeJSON(handle.readAll())
            os.queueEvent("redraw_screen")
          end
