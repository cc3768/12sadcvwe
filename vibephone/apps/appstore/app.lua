...
local function installBundle(bundle)
  ...
  if id ~= "" then
    ...
    for _,v in ipairs(phoneState.apps.order) do if v == id then found = true end end
    if not found then
      phoneState.apps.order[#phoneState.apps.order+1] = id
...
local function handleServer(msg)
  ...
  if msg.t == "app_bundle" then
    ...
    if ok then
      ...
      S.status = "Install failed: "..tostring(msg.id)
    end
    return
  end
...
local function wsLoop()
  while true do
    if not S.connected then
      connect()
      if S.connected then requestList() end
      os.sleep(0.5)
    else
      -- keepalive ping (optional; server can ignore)
      if (os.clock() - S.lastPing) >= S.pingEvery then
        wsSend({ t="ping", ts=os.epoch("utc") })
        S.lastPing = os.clock()
      end

      local ok, msg = pcall(function() return S.ws.receive(2) end) -- 2s timeout
      if not ok or msg == nil then
        -- timeout is fine; loop continues
      else
  ...
  while true do
    ...
    elseif e == "key" then
      ...
      elseif a == keys.escape then
        if S.detail then S.detail = false else safeClose(); return end
      end
    end
  end
end

parallel.waitForAny(wsLoop, uiLoop)
